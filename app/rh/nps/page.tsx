...
export default function NPSPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-white">NPS</h1>
      <p className="mt-1 text-gray-300">Net Promoter Score - Pesquisa de satisfação</p>
      <div className="mt-6 flex gap-2">
        ...
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm ${aba === "historico" ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-300 hover:bg-gray-600"}`}
        >
          Histórico
        </button>
      </div>
        ...
        <div className="mt-8 space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            <div className="card-white p-6 text-center">
              ...
              <p className="mt-1 text-sm text-gray-400">NPS Score</p>
            </div>
            <div className="card-white p-6 text-center">
              <p className="text-4xl font-bold text-green-400">{promotores}</p>
              <p className="mt-1 text-sm text-gray-400">Promotores (9-10)</p>
            </div>
            <div className="card-white p-6 text-center">
              <p className="text-4xl font-bold text-yellow-400">{neutros}</p>
              <p className="mt-1 text-sm text-gray-400">Neutros (7-8)</p>
            </div>
            <div className="card-white p-6 text-center md:col-span-3">
              <p className="text-4xl font-bold text-red-400">{detratores}</p>
              <p className="mt-1 text-sm text-gray-400">Detratores (0-6)</p>
            </div>
          </div>
          <div className="card-white overflow-hidden">
            <h2 className="card-white-header text-lg font-semibold text-white">Respostas recentes</h2>
            <table className="w-full divide-y divide-blue-500/30">
              <thead className="bg-gray-800/50">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-medium text-white">Nota</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-white">Comentário</th>
      ...
      {aba === "pesquisas" && (
        <div className="mt-6 card-white p-6">
          <button className="rounded-lg bg-ecosystem-red px-4 py-2 text-white hover:opacity-90">
            Nova Pesquisa NPS
          </button>
          <p className="mt-4 text-gray-400">Configure e envie pesquisas NPS para os colaboradores.</p>
        </div>
      )}
      {aba === "historico" && (
        <div className="mt-6 card-white p-6 text-gray-400">
